...
class TeachersAdapterNew: RecyclerView.Adapter<TeachersAdapterNew.TeacherViewHolder>() {
    ...
    class TeacherViewHolder(
        ...
        fun bind(
            teacher: TeacherWithSpeciality,
            position: Int,
            allTeachers: List<TeacherWithSpeciality> // Добавляем список всех преподавателей
        ) {
            teacherNameTextView.text = "Преподаватель: ${teacher.teacher.name}"

            var totalHours = 0
            for(item in allTeachers){
                if(teacher.teacher.name==item.teacher.name)
                    totalHours+=teacher.teacher.hoursPerYear;
            }
            if(totalHours>1440){
            ...
            teacherHours.text = "Количество часов: ${teacher.teacher.hoursPerYear}"
            teacherSpeciality.text = "Специальность: ${teacher.teacher.speciality}"
